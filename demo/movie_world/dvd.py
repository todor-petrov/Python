class DVD:

    def __init__(self, name: str, dvd_id: int, creation_year: int, creation_month: str, age_restriction: int):
        self.name = name
        self.id = dvd_id
        self.creation_year = creation_year
        self.creation_month = creation_month
        self.age_restriction = age_restriction
        self.is_rented = False

    @classmethod
    def from_date(cls, dvd_id: int, name: str, date: str, age_restriction: int):
        months = {1: 'January', 2: 'February', 3: 'March', 4: 'April', 5: 'May', 6: 'June',
                  7: 'July', 8: 'August', 9: 'September', 10: 'October', 11: 'November', 12: 'December'}
        day, month_as_integer, year = [int(x) for x in date.split('.')]
        month = months[month_as_integer]
        return cls(name, dvd_id, year, month, age_restriction)

    def __repr__(self):
        status_rented = 'rented' if self.is_rented else 'not rented'
        return (f'{self.id}: {self.name} ({self.creation_month} {self.creation_year}) '
                f'has age restriction {self.age_restriction}. Status: {status_rented}')
